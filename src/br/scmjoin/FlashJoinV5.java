...
public class FlashJoinV5 {
	...
	TreeMap<Double, ArrayList<String>> fetchMap = new TreeMap<Double, ArrayList<String>>();
	...
	int atribReRead = 0;
	static String path = "C:/TPCH10/";

	public static void main(String[] args) throws Exception {
		FlashJoinV5 fjV5 = new FlashJoinV5();
		...
		if (teste.equals("Q5")) {
			...
			flobj01.setIntermediateTableJoin(data + "intermediateTableQ5.b");
			...
			flobj02.setTable1(data + "NextJoinQ5.b");

			flobj02.setTable2(data + "customer.b");

			flobj02.setIntermediateTableJoin(data + "intermediateTableQ5.b");

			flobj02.setMemorySizeJoinKernel(memorySize);
			flobj02.setMemorySizeFetchKernel(memorySizeFetchKernel);
			...
			flobj03.setTable1(data + "NextJoinQ5.b");

			flobj03.setTable2(data + "orders.b");

			flobj03.setIntermediateTableJoin(data + "intermediateTableQ5.b");

			flobj03.setMemorySizeJoinKernel(memorySize);
			flobj03.setMemorySizeFetchKernel(memorySizeFetchKernel);
			...
			flobj04.setTable1(data + "NextJoinQ5.b");

			flobj04.setTable2(data + "lineitem.b");

			flobj04.setIntermediateTableJoin(data + "intermediateTableQ5.b");

			flobj04.setMemorySizeJoinKernel(memorySize);
			flobj04.setMemorySizeFetchKernel(memorySizeFetchKernel);
			...
			flobj05.setTable1(data + "NextJoinQ5.b");

			flobj05.setTable2(data + "supplier.b");

			flobj05.setIntermediateTableJoin(data + "intermediateTableQ5.b");

			flobj05.setMemorySizeJoinKernel(memorySize);
			flobj05.setMemorySizeFetchKernel(memorySizeFetchKernel);
		...
		if (teste.equals("Q10")) {
			...
			flobj01.setIntermediateTableJoin(data + "intermediateTableQ10.b");
			...
			flobj02.setTable1(data + "NextJoinQ10.b");

			flobj02.setTable2(data + "lineitem.b");

			flobj02.setIntermediateTableJoin(data + "intermediateTableQ10.b");
			...
			flobj03.setTable1(data + "NextJoinQ10.b");

			flobj03.setTable2(data + "nation.b");

			flobj03.setIntermediateTableJoin(data + "intermediateTableQ10.b");
...
//			System.out.println("JOIN 1 = ORDER x Lineitem");
			...
			flobj01.kernel.tableFromJoinKernel = data + "intermediateTableQ3.b";
			flobj01.kernel.tablesReRead = new String[] { data + "orders.b" };
			flobj01.kernel.ColReRead = new String[] { "orders" };
			flobj01.kernel.atbReRead = new String[] { "orderkey" };
			flobj01.kernel.lastJoin = false;
...
//			System.out.println("JOIN 2 = Customer x Orders x Lineitem");
			...
			flobj02.setTable1(data + "NextJoinQ3.b");

			flobj02.setTable2(data + "lineitem.b");

			flobj02.setIntermediateTableJoin(data + "intermediateTableQ3.b");
		...
		if (teste.equals("QA")) {
			...
			resultlog = new LogFile(result + "QA.txt");
		...
		if (teste.equals("QB")) {
			...
			resultlog = new LogFile(result + "QB.txt");
	...
	private void flashJoinV5(ArrayList<FlashObj> listjoin) throws Exception {
		...
		for (int w = 0; w < listjoin.size(); w++) {
			...
			hafFlashJoin.create(join.intermediateTableJoin, join.headerJoin);
...
//			 hafFlashJoin.numberofWrite);
			...
			HandleFile haf5 = new HandleFile(blockSize);
			...
			for (int i = 0; i < join.kernel.tablesReRead.length; i++) {
				if (i == 0) {
					haf0.open(join.kernel.tablesReRead[i]);
				}
				if (i == 1) {
					haf1.open(join.kernel.tablesReRead[i]);
				}
				if (i == 2) {
					haf2.open(join.kernel.tablesReRead[i]);
				}
				if (i == 3) {
					haf3.open(join.kernel.tablesReRead[i]);
				}
				if (i == 4) {
					haf4.open(join.kernel.tablesReRead[i]);
				}
				if (i == 5) {
					haf5.open(join.kernel.tablesReRead[i]);
				}
			}
...
//				System.out.println(key);
			...
			System.out.println(contagemFetch);
			...
			for (int i = 0; i < join.kernel.tablesReRead.length; i++) {
				if (i == 0) {
					...
					tuple_DB = tuple_DB + haf0.numberofReReadbyRowId + "|";
				...
				if (i == 1) {
					...
					tuple_DB = tuple_DB + haf1.numberofReReadbyRowId + "|";
					// resultlog.writeLog("ReRead FetchKernel "
					// + join.kernel.tablesReRead[i] + ": "
				...
				if (i == 2) {
					...
					tuple_DB = tuple_DB + haf2.numberofReReadbyRowId + "|";
					// resultlog.writeLog("ReRead FetchKernel "
					// + join.kernel.tablesReRead[i] + ": "
				...
				if (i == 3) {
					...
					tuple_DB = tuple_DB + haf3.numberofReReadbyRowId + "|";
					// resultlog.writeLog("ReRead FetchKernel "
					// + join.kernel.tablesReRead[i] + ": "
				...
				if (i == 4) {
					...
					tuple_DB = tuple_DB + haf4.numberofReReadbyRowId + "|";
					// resultlog.writeLog("ReRead FetchKernel "
					// + join.kernel.tablesReRead[i] + ": "
				...
				if (i == 5) {
					releiturasAcumuladas += haf5.numberofReReadbyRowId;
					tuple_DB = tuple_DB + haf5.numberofReReadbyRowId + "|";
					// resultlog.writeLog("ReRead FetchKernel "
					// + join.kernel.tablesReRead[i] + ": "
					// + haf5.numberofReReadbyRowId);
					haf5.close();
				}
			}
	...
	private void addToFetch(String linhaToAux2, FlashObj join,
		...
		Integer x = Integer.valueOf(linhaSplit[0].substring(3, 11));
		sort = x + sort;
		if (join.kernel.lastJoin) {
			for (int i = 1; i < join.kernel.tablesReRead.length; i++) {
				x = Integer.valueOf(linhaSplit[i].substring(3, 11));
				auxSort = x / (40000.0 * i);
				sort += auxSort;
			}
		}
		ArrayList<String> auxFetchMap = new ArrayList<String>();
		calcMemorySortFetch(join, linhaSplit.length);
		if (join.memorySizeFetchKernel > memoryUsedInFetch) {
			if (fetchMap.containsKey(sort)) {
				auxFetchMap = fetchMap.get(sort);
				numberoftupleInFetch++;
			}
			auxFetchMap.add(linhaToAux2);
			fetchMap.put(sort, auxFetchMap);
		} else {
			sendToDiskFetch(hafFetch);
			if (fetchMap.containsKey(sort)) {
				auxFetchMap = fetchMap.get(sort);
				numberoftupleInFetch++;
			}
	...
	private void sendToDiskFetch(HandleFile hafFetch) throws Exception {
		ArrayList<String> fetchToDisk = new ArrayList<>();
		for (Double key : fetchMap.keySet()) {
			fetchToDisk = fetchMap.get(key);
			for (int i = 0; i < fetchToDisk.size(); i++) {
				hafFetch.writeTuple(fetchToDisk.get(i));
				numberoftupleInFetch--;
			}
		}
	...
	private void sendToDiskBigger(byte[] tupleBlockToMemory, HandleFile haf,
		...
		for (Integer hashResultInMapForSmaller : mapOfTableSmaller.keySet()) {
			...
			if (bucketsInMemoryForBigger.containsKey(hashResultInMapForSmaller)) {
				...
				auxBucketsInMemory = new THashMap<ByteBuffer, ArrayList<byte[]>>();
				bucketsInMemoryForBigger.put(hashResultInMapForSmaller, auxBucketsInMemory);
				bucketsInMemoryForBigger.remove(hashResultInMapForSmaller);
	...
	private void sendToMemorySmaller(byte[] tupleBlockToMemory, HandleFile haf,
		...
		if (join.firstJoinTable1) {
			rowid = haf.getRowid(indCol);
			String x = RafIOCalc.getRowid(rowid);
		} else {
			rowid = tupleBlockToMemory;
		}
